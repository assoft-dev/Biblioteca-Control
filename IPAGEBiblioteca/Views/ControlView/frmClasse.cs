namespace IPAGEBiblioteca.Views.ControlView
{
    ...
    {
        #region Declaracoes
        private readonly IClasseModels userModels;
        #endregion

        #region Construtores da Classe
        public frmClasse(IClasseModels userModels)
        {
            InitializeComponent();
        ...
        {
            var userdelete = classeModelsBindingSource.Current as ClasseModels;
            if (userdelete != null)
            {
        ...
        private void MenuEditar_Click(object sender, System.EventArgs e)
        {
            ...
            {
                using (var forms = new frmClasseAdd(classeModelsBindingSource.Current as ClasseModels,
        ...
        {
            using (var forms = new frmClasseAdd(null,
                                                new ClasseRepository(new BiblioteContext())))
            {
                var usercontrol = OpenFormsDialog.ShowForm(null, forms);
                if (usercontrol == DialogResult.OK)
        ...
        private async void Loader()
        {
            ...
            try
            {
                classeModelsBindingSource.DataSource = await userModels.GetAll()
                                                                      .ToListAsync();
            }
            finally
            {
