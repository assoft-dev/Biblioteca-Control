namespace IPAGEBiblioteca.Views.ControlView
{
    ...
    public partial class frmLivros : XtraUserControl
    {
        #region Declaracoes
        private readonly ILivrosModels userModels;
        #endregion
        ...
        {
            var userdelete = livrosModelsBindingSource.DataSource as List<LivrosModels>;
            if (userdelete.Count > 0)
            {
                ReportControllers.GetReport(new rptLivros(userdelete), false);
            }
        }
        private async void MenuApagar_Click(object sender, System.EventArgs e)
        {
            var userdelete = livrosModelsBindingSource.Current as LivrosModels;
            if (userdelete != null)
            {
        ...
        private void MenuEditar_Click(object sender, System.EventArgs e)
        {
            ...
            {
                using (var forms = new frmLivrosAdd(
                                                     livrosModelsBindingSource.Current as LivrosModels,
        ...
        {
            using (var forms = new frmLivrosAdd(null,
                                                new LivrosRepository(new BiblioteContext()),
                                                new AutorRepositoty(new BiblioteContext()),
                                                new EditoraRepository(new BiblioteContext())))
            {
        ...
        private async void Loader()
        {
            ...
            try
            {
                livrosModelsBindingSource.DataSource = await userModels.GetAll()
                                                                  .Include(x => x.autoModels)
                                                                  .Include(x => x.EditoraModels)
                                                                  .ToListAsync();
            }
            finally
            {
