namespace IPAGEBiblioteca.Views.ControlView
{
    ...
    public partial class frmDefinicoesAdd : XtraUserControl
    {
        private readonly IDefinicoesModels definicoesModels;

        public frmDefinicoesAdd(IDefinicoesModels definicoesModels)
        {
            InitializeComponent();
            this.definicoesModels = definicoesModels;
            ...
            definicoesModelsBindingSource.AddNew();
            NovoDado();
        }
        ...
        private void Loader()
        {
            definicoesModelsBindingSource.DataSource = definicoesModels.GetAll().ToList();
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(DefinicoesTituloTextEdit.Text))
            {
                ...
                DefinicoesTituloTextEdit.Focus();
                return false;
            }
            int i = 0;
            foreach (var item in definicoesModelsBindingSource.DataSource as List<DefinicoesModels>)
            {
                i = i + 1;
                if (string.IsNullOrWhiteSpace(item.DefinicoesTitulo))
                {
        ...
        private async void GuardarDados()
        {
            if (Validacao())
            {
                var permissoes = definicoesModelsBindingSource.DataSource as List<DefinicoesModels>;
                if (permissoes != null)
                {
                    var result = await definicoesModels.Insert(permissoes);
                    if (result)
        ...
        private async void ApagarDados()
        {
                ...
                "Perda de Dados Permanente", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                try
                {
                    if (await definicoesModels.Delete(new DefinicoesModels { ID = (int)IDTextEdit.EditValue, }))
                    {
