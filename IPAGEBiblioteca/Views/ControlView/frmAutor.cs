namespace IPAGEBiblioteca.Views.ControlView
{
    ...
    public partial class frmAutor : XtraUserControl
    {
        #region Declaracoes
        private readonly IAutorModels userModels;
        #endregion

        #region Construtores da Classe
        public frmAutor(IAutorModels userModels)
        {
            InitializeComponent();
        ...
        {
            var userdelete = autorModelsBindingSource.Current as AutorModels;
            if (userdelete != null)
            {
        ...
        private void MenuEditar_Click(object sender, System.EventArgs e)
        {
            ...
            {
                using (var forms = new frmAutorAdd(autorModelsBindingSource.Current as AutorModels,
        ...
        {
            using (var forms = new frmAutorAdd(null,
                                                new AutorRepositoty(new BiblioteContext())))
            {
                var usercontrol = OpenFormsDialog.ShowForm(null, forms);
                if (usercontrol == DialogResult.OK)
        ...
        private async void Loader()
        {
            ...
            try
            {
                autorModelsBindingSource.DataSource = await userModels.GetAll()
                                                                       .ToListAsync();
            }
            finally
            {
