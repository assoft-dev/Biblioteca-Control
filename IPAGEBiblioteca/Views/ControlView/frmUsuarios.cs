namespace IPAGEBiblioteca.Views.ControlView
{
    ...
    public partial class frmUsuarios : XtraUserControl
    {
        ...
        {
            var userdelete = userModelsBindingSource.DataSource as List<UserModels>;
            if (userdelete.Count > 0)
            {
                ReportControllers.GetReport(new rptUsuarios(userdelete), false);
            }
        }
        private async void MenuApagar_Click(object sender, System.EventArgs e)
        {
            var userdelete = userModelsBindingSource.Current as UserModels;
            if (userdelete != null)
            {
        ...
        private void MenuEditar_Click(object sender, System.EventArgs e)
        {
            ...
            {
                using (var forms = new frmUsuariosAdd(
                                                     userModelsBindingSource.Current as UserModels,
                                                     new UserRepository(new BiblioteContext()),
        ...
        {
            using (var forms = new frmUsuariosAdd(null, 
                                                  new UserRepository(new BiblioteContext()),
                                                  new GruposRepository(new BiblioteContext())))
            {
                var usercontrol = OpenFormsDialog.ShowForm(null, forms);
        ...
        private async void Loader()
        {
            ...
            try
            {
                userModelsBindingSource.DataSource = await userModels.GetAll()
                                                                  .Include(x => x.GruposModels)
